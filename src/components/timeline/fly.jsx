...
//       /* ---- YEAR (outlined, above card) ---- */
//       const YEAR_ZONE_H = 150;
...
export default function Fly() {
  ...
  useEffect(() => {
    ...
    */
    ...
    function buildCardMeshes() {
      ...
      waypointData.forEach(
        (data, index) => {
          ...
          /*
            Orient the card using the
            curve's own tangent frame at
            this waypoint, instead of a
            fixed world-space rotation.

            This keeps every card "straight"
            relative to the flight path,
            even where the path curves
            left/right, instead of only
            looking correct on straight
            stretches.
          */
          const cardFrame =
            getCurveFrame(
    ...
    /*
      Reusable vectors/quaternions for
      orienting each card to face the
      camera directly (billboard), so
      cards read as straight/flat-on
      instead of keystoned when the
      camera approaches at an angle.
    */
    const cardForward =
      new THREE.Vector3();
    ...
    const cardBasisMatrix =
      new THREE.Matrix4();

    const cardOrientQuat =
      new THREE.Quaternion();

    const CARD_LOCAL_TILT =
      new THREE.Quaternion().setFromAxisAngle(
        new THREE.Vector3(
          1,
          0,
      ...
      cardMeshes.forEach(
        (mesh) => {
          ...
          */
          ...
          /*
            Billboard the card toward the
            camera's current position every
            frame, so it always reads as
            straight/flat-on to the viewer
            instead of keystoning as the
            chase camera approaches from an
            angle. A small local tilt is
            kept for the cinematic lean.
          */
          cardForward
            .copy(
              camera.position
          ...
          cardBasisMatrix.makeBasis(
            cardRight,
            cardUp,
            cardForward
          );

          cardOrientQuat.setFromRotationMatrix(
            cardBasisMatrix
          );

          cardOrientQuat.multiply(
            CARD_LOCAL_TILT
          );
