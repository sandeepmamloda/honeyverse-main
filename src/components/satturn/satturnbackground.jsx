...
//     // On narrow/portrait screens a fixed FOV + fixed distance crops the rings.
//     // We widen the FOV and pull the camera back as the aspect ratio gets
//     // narrower (more "portrait"), and bump it further on mobile devices.
//     function getResponsiveCameraSetup() {
...
//       if (aspect < 1) {
//         // Portrait (phones). The narrower the aspect, the more we need to
//         // back off / widen so the full ring width stays in frame.
//         const portraitFactor = Math.min(2.4, 1 / aspect); // e.g. 9:16 -> ~1.78
//         fov = Math.min(72, BASE_FOV * (1 + (portraitFactor - 1) * 0.6));
//         z = BASE_Z * (1 + (portraitFactor - 1) * 0.55);
//       } else if (aspect < 1.35) {
//         // Square-ish / small tablets in portrait-adjacent ratios
//         fov = 52;
//         z = BASE_Z * 1.15;
//       }

//       if (isMobile) {
//         // Slight extra breathing room for small physical screens
//         z *= 1.05;
//       }
...
//     const initialCam = getResponsiveCameraSetup();
...
//       1000
//     );
//     camera.position.set(0, 1.6, initialCam.z);
...
//         vViewPos = mvPosition.xyz;
//         float shimmer = 0.9 + 0.1 * sin(uTime * 1.6 + twinkle * 6.2831);
//         gl_PointSize = size * shimmer * uSizeBoost * (340.0 / -mvPosition.z);
//         gl_Position = projectionMatrix * mvPosition;
//       }
//     `;
//     const planetFragmentShader = `
...
//         uRingInner: { value: 2.55 },
//         uRingOuter: { value: 4.75 },
//         uSizeBoost: { value: isMobile ? 1.25 : 1.0 },
//       },
//       vertexShader: planetVertexShader,
//       fragmentShader: planetFragmentShader,
//       transparent: true,
...
//         vViewPos = mvPosition.xyz;
//         float shimmer = 0.9 + 0.1 * sin(uTime * 1.6 + twinkle * 6.2831);
//         gl_PointSize = size * shimmer * uSizeBoost * (340.0 / -mvPosition.z) * 2.3;
//         gl_Position = projectionMatrix * mvPosition;
//       }
//     `;
//     const planetBloomFragmentShader = `
...
//         vec4 mvPosition = modelViewMatrix * vec4(position, 1.0);
//         float shimmer = 0.88 + 0.12 * sin(uTime * 1.8 + twinkle * 6.2831);
//         gl_PointSize = size * shimmer * uSizeBoost * (320.0 / -mvPosition.z);
//         gl_Position = projectionMatrix * mvPosition;
//       }
//     `;
//     const ringFragmentShader = `
...
//         vec4 mvPosition = modelViewMatrix * vec4(position, 1.0);
//         float shimmer = 0.88 + 0.12 * sin(uTime * 1.8 + twinkle * 6.2831);
//         gl_PointSize = size * shimmer * uSizeBoost * (320.0 / -mvPosition.z) * 2.1;
//         gl_Position = projectionMatrix * mvPosition;
//       }
//     `;
//     const ringBloomFragmentShader = `
...
//     // Slightly larger base scale on mobile so the system fills more of a
//     // narrow viewport instead of looking small and lost in the middle.
//     const MOBILE_SCALE_BOOST = isMobile ? 1.18 : 1.0;
//     system.scale.setScalar(0.001);
...
//     // Mutable so resize/orientation changes can update the "home" camera
//     // distance and ring tilt without breaking the scroll-dolly math.
...
//         Math.max(0.001, entranceScale * scrollScale * MOBILE_SCALE_BOOST)
//       );
...
//       // Recompute FOV + distance for the new aspect ratio (handles phone
//       // rotation between portrait/landscape and browser chrome changes).
//       const { fov, z } = getResponsiveCameraSetup();
//       camera.fov = fov;
//       baseCameraZ = z;
...
//     };
//     window.addEventListener("resize", handleResize);
//     // Some mobile browsers fire resize late after rotation; re-check shortly after.
...
export default function SaturnBackground() {
  ...
  useEffect(() => {
    ...
    // Lowered from the previous pass. This is the single biggest lever for
    // both initial build time AND steady-state frame lag (fewer verts to
    // upload/shade every frame). Visual density barely changes because the
    // points themselves are small and overlap a lot.
    const QUALITY = {
      planetCount: isLowPower ? 16000 : 55000,
      ringCount: isLowPower ? 20000 : 65000,
      haloCount: isLowPower ? 500 : 1400,
      pixelRatioCap: isLowPower ? 1.5 : 2, // 3x on mobile GPUs was the main lag source
    };
    ...
    function getResponsiveCameraSetup() {
      ...
      if (aspect < 1) {
        const portraitFactor = Math.min(2.4, 1 / aspect);
        fov = Math.min(72, BASE_FOV * (1 + (portraitFactor - 1) * 0.6));
        z = BASE_Z * (1 + (portraitFactor - 1) * 0.55);
      } else if (aspect < 1.35) {
        fov = 52;
    ...
    // ---------- DEFERRED HEAVY INIT ----------
    // Building ~70-120k-point geometries synchronously inside useEffect
    // blocks the first paint. Deferring with requestIdleCallback lets the
    // page/layout paint first, then the scene builds in the background.
    ...
    function init() {
      ...
      const renderer = new THREE.WebGLRenderer({
        antialias: !isLowPower, // AA is expensive per-frame on weak mobile GPUs
        alpha: false,
      ...
      // Octaves 4 -> 3. Called twice per planet vertex during generation;
      // this alone meaningfully cuts build time with negligible visual loss.
      function fbm(x, y, z) {
        let v = 0,
          amp = 0.5,
          freq = 1.0;
      ...
      function buildRings() {
        ...
        let written = 0,
          attempts = 0;
        // Lowered from count * 8. The rejection-sampling loop was the other
        // big chunk of build time; fewer max attempts = faster bail-out
        // with an imperceptible difference in final ring density.
      ...
      // rAF-throttled scroll handler: avoids doing this work more than once
      // per frame on browsers that fire many scroll events in a burst.
      const handleScroll = () => {
      ...
      const handleMouseMove = (e) => {
        ...
        mouseTicking = true;
        requestAnimationFrame(() => {
          mouseX = (pendingClientX / window.innerWidth) * 2 - 1;
          mouseY = (pendingClientY / window.innerHeight) * 2 - 1;
          mouseTicking = false;
        });
      };
      if (supportsHover) {
      ...
      // Preallocated scratch objects reused every frame instead of being
      // cloned each tick. Repeated .clone() calls inside animate() were a
      // steady source of garbage-collector pauses (a common cause of
      // perceived "lag"/stutter in long-running WebGL loops).
      const scratchViewLightDir = new THREE.Vector3();
      const scratchLocalLightDir = new THREE.Vector3();
      const systemInverseQuat = new THREE.Quaternion();
      const worldLightDir = lightDir.clone();

      function updateLightUniforms() {
        scratchViewLightDir.copy(lightDir).transformDirection(camera.matrixWorldInverse);
        planetMaterial.uniforms.uLightDir.value.copy(scratchViewLightDir);

        systemInverseQuat.copy(system.quaternion).invert();
        scratchLocalLightDir.copy(worldLightDir).applyQuaternion(systemInverseQuat);
        planetMaterial.uniforms.uLightDirWorld.value.copy(scratchLocalLightDir);
      }
